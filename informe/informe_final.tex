...
Kalos Lazo \quad Aaron Navarro \quad Fernando Usurin\\[0.15cm]
...
\For{$\text{stride}=1$ \textbf{to} $\log_2 p$}
  \ForAll{$k=0,2\cdot\text{stride},\dots$ \textbf{pardo}}
...
estructural $E\,n\,d\,h$ a segundos (refleja la velocidad de la máquina) y $b$ la
latencia asociada a cada ronda de la reducción \emph{arg\-máx}. Ambas se estiman
...
$<1.5\%$: el cuello no es la comunicación.}
\end{figure}

El tiempo cae con $p$ sin saturarse prematuramente. La reducción
$T_{\text{reduce}}$ es $<1.5\%$ de $T_{\text{total}}$, confirmando el carácter
...
predicción en la región baja: $S(p)$ sigue la diagonal ideal hasta $p\approx 8$
(eficiencia $E>0.7$), lo que valida tanto el reparto de semillas como la
hipótesis de trabajo dominante. Más allá de ese punto el crecimiento se atenúa y
...
($T_{\text{reduce}}<1.5\%$) sino a dos factores del hardware: el
...
procesador adicional. Bajo \emph{strong scaling} decrece de forma sostenida con
$p$, pasando de $\sim$0.99 en $p=2$ a $\sim$0.33 en $p=32$, reflejando los mismos
...
región lineal, mientras que la leve desviación a alto $p$ corresponde a la
contención de memoria que el modelo lineal no captura (y no a la reducción, cuyo
peso medido es $<1.5\%$). El elevado $R^2$ permite concluir que la expresión
...
operaciones: crece de forma sostenida hasta estabilizarse en torno a
\textbf{$\sim$76 GFLOP/s} para $p\ge 16$. Que la curva se sature al mismo nivel
